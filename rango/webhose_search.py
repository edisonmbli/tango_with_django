...
def run_query(search_terms, size=10):
    ...
    search_url = (
        '{root_url}?token={key}&format=json&q={query}&sort=relevancy&size={size}').format(
        root_url=root_url, key=webhose_api_key, query=query_string, size=size)
    ...
    try:
        ...
        for post in json_response['posts']:
            results.append(
                {'title': post['title'], 'link': post['url'], 'summary': post['text'][:200]})
    except:
